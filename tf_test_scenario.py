...
x = tf.placeholder(tf.float32, shape=[None, 64,32, 3])
y = tf.placeholder(tf.float32, shape=[None, 1])

xIn = tf.reshape(x, shape=[-1, inSize ]) # flatten
fc_1w = tf.Variable(tf.random_normal([inSize, 1], stddev=0.01))
fc_1b   = tf.Variable(tf.random_normal([1], stddev=0.01))

fc1 = tf.add(tf.matmul(xIn, fc_1w), fc_1b)
fc1 = tf.nn.tanh(fc1)
fc1 = tf.nn.dropout(fc1, 0.5) # plenty of dropout...

fc_2w = tf.Variable(tf.random_normal([1, inSize], stddev=0.01))  # back to input size
fc_2b = tf.Variable(tf.random_normal([inSize], stddev=0.01))

y_pred = tf.add(tf.matmul(fc1, fc_2w), fc_2b)
#y_pred = tf.reshape( y_pred, shape=[-1, 64, 32, 3])
...
for epoch in range(trainingEpochs):
	#c = (epoch * batchSize) % 1 #vel.shape[0]
#	batch = []
...
#    batch.append(vel)
	...
	x_batch.append(vel)
	y_batch=[]
	y_batch.append(np.ones(1))	
	_ , currentCost = sess.run([opt, cost], feed_dict={x: x_batch, y: y_batch})
	#print("Epoch %d/%d: cost %f " % (epoch, trainingEpochs, currentCost) ) # debug, always output cost
    
	if epoch%10==9 or epoch==trainingEpochs-1:
		[valiCost,vout] = sess.run([cost, y_pred], feed_dict={x: x_valiData, y: y_valiData})
		print("Epoch %d/%d: cost %f , validation cost %f " % (epoch, trainingEpochs, currentCost, valiCost) )

		if epoch==trainingEpochs-1:
			print("\n Training done. Writing %d images from validation data to current directory..." % len(x_valiData) )
			print(vout.shape)
			for i in range(len(x_valiData)):
				scipy.misc.toimage( np.reshape(x_valiData[i], [64, 32,3]) , cmin=0.0, cmax=1.0).save("in_%d.png" % i)
				scipy.misc.toimage( np.reshape(vout[i]    , [64, 32,3]) , cmin=0.0, cmax=1.0).save("out_%d.png" % i)
