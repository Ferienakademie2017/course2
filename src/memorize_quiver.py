...
if __name__ == "__main__":
    ...
    U = image[::2,::2,0]
    V = image[::2,::2,1]
    plt.quiver(X,Y,U,V,headwidth=3,headlength=5)
    plt.xlim(0, image_size[0])
    plt.ylim(0, image_size[1])
    
    # streamlines version    
    #X,Y = np.mgrid[0:1:image_size[0], 0:1:image_size[1]]
    #U = image[:,:,0]
    #V = image[:,:,1]

    #fig0, ax0 = plt.subplots()
    #strm = ax0.streamplot(X, Y, U, V, linewidth=2, cmap=plt.cm.autumn)
